                        ...
                        <form method="GET" action="{{ route('domestic.shipment.reports') }}" id="filterForm">
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select class="form-control form-select" name="consigner_id">
                                        <option value="">All Consigners</option>
                                        @foreach ($consigners ?? [] as $consigner)
                                            <option value="{{ $consigner->id }}"
                                                {{ request('consigner_id') == $consigner->id ? 'selected' : '' }}>
                                                {{ $consigner->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select class="form-control form-select" name="consignee_id">
                                        <option value="">All Consignees</option>
                                        @foreach ($consignees ?? [] as $consignee)
                                            <option value="{{ $consignee->id }}"
                                                {{ request('consignee_id') == $consignee->id ? 'selected' : '' }}>
                                                {{ $consignee->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-3">
                                    ...
                                    <div class="input-group flatpickr" id="startDatePicker">
                                        <input type="text" class="form-control" name="start_date"
                                            placeholder="YYYY-MM-DD" value="{{ request('start_date') }}" data-input>
                                    ...
                                    <div class="input-group flatpickr mt-1" id="endDatePicker">
                                        <input type="text" class="form-control" name="end_date" placeholder="YYYY-MM-DD"
                                            value="{{ request('end_date') }}" data-input>
                            ...
                            <div class="row mt-3">
                                <div class="col-md-12">
                                    ...
                                    <a href="{{ route('domestic.shipment.reports') }}" class="btn btn-secondary">Clear
                                        Filters</a>
                                </div>
                            </div>
                        </form>
                                    ...
                                    <tbody>
                                        @forelse($shipments as $key => $row)
                                            <tr class="{{ $row->is_profit ? 'table-success' : 'table-danger' }}">
                                                ...
                                                <td class="text-center">
                                                    <span
                                                        class="badge bg-{{ $row->vehicle_type === 'rented' ? 'info' : 'secondary' }}">
                                                        {{ ucfirst($row->vehicle_type ?? 'own') }}
                                                    </span>
                                                </td>

                                                <td class="text-center">
                                                    {{ $row->vehicle_type === 'rented' ? $row->vehicleHire?->hire_register_id ?? '-' : 'OWN' }}
                                                </td>
                                                ...
                                                <td
                                                    class="text-end fw-bold {{ $row->is_profit ? 'text-success' : 'text-danger' }}">
                                    ...
                                    @if ($shipments->count())
                                        <tfoot class="table-light fw-bold">
                                            <tr>
                                                ...
                                                <td
                                                    class="text-end {{ $shipments->sum('profit_loss') >= 0 ? 'text-success' : 'text-danger' }}">
